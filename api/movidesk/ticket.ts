...
function parseTicket(ticket: any, requestedId: string) {
  ...
  if (!cnpj) {
    const cnpjMatch = allText.match(/\b\d{2}\.?\d{3}\.?\d{3}\/?\d{4}-?\d{2}\b|\b\d{14}\b/);
    cnpj = cnpjMatch?.[0] || '';
  }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Compatibilidade com APKs já gerados: aceita token recebido, mas prioriza o segredo do servidor.
    ...
    if (!token) {
      return res.status(503).json({
        error: 'MOVIDESK_API_TOKEN não está configurado no servidor Vercel.',
      });
    }
    ...
    // Protocolos Movidesk costumam ser números longos. Consulte protocolo primeiro para evitar filtro de ID inválido.
    ...
    if (protocolResult.response.ok && Array.isArray(protocolResult.data) && protocolResult.data.length) {
      ticket = protocolResult.data[0];
    } else if (!protocolResult.response.ok) {
      upstreamErrors.push(`protocol:${protocolResult.response.status} ${protocolResult.text.slice(0, 250)}`);
    }

    // Se não encontrou por protocolo e o valor cabe como ID numérico seguro, tente ID direto.
    if (!ticket && /^\d+$/.test(id) && id.length <= 10) {
      ...
      if (idResult.response.ok) {
        ...
        upstreamErrors.push(`id:${idResult.response.status} ${idResult.text.slice(0, 250)}`);
      }
    }
    ...
    if (!ticket) {
      if (upstreamErrors.length) {
        return res.status(502).json({
          error: 'O Movidesk recusou a consulta. Verifique o token e as permissões da API.',
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : 'Erro interno ao consultar o Movidesk.',
    });
  }
}
