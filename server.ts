...
async function startServer() {
  ...
  app.get("/api/movidesk/ticket", async (req, res) => {
    try {
      ...
      // 2. If not found, try protocol/id filter fallback
  ...
  app.post("/api/movidesk/export", async (req, res) => {
    try {
      ...
      if (!updateRes.ok) {
        ...
        // If status update failed due to Movidesk status/reason rules, retry without status to ensure laudo action is still added
